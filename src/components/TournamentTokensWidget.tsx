...
export default function TournamentTokensWidget({
  ...
  if (loading) {
    return (
      <div className="bg-linear-to-r from-[#8F60D0]/20 to-purple-500/20 rounded-lg p-4 border border-[#8F60D0]/30 animate-pulse">
  ...
  return (
    <div
      className={`bg-linear-to-r rounded-lg p-4 border transition-all duration-300 ${
        isOutOfTokens
      ...
      <div className="flex items-center gap-3">
        <div
          className={`p-2 rounded-full ${
            isOutOfTokens
              ...
              : isLowTokens
                ? "bg-orange-500"
                : "bg-linear-to-r from-[#8F60D0] to-purple-500"
          }`}
        >
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between">
            ...
            <span
              className={`font-bold ${
                isOutOfTokens ? "text-red-300" : isLowTokens ? "text-orange-300" : "text-green-300"
              }`}
            >
          ...
          <div className="text-sm text-white mt-1">
            {tokensInfo.plan} • {tokensInfo.usedTokens} utilisé
            {tokensInfo.usedTokens > 1 ? "s" : ""} ce mois
          </div>
          ...
          {isLowTokens && !isOutOfTokens && (
            <div className="text-xs text-orange-300 mt-1">
              Attention: {tokensInfo.remainingTokens} token
              {tokensInfo.remainingTokens > 1 ? "s" : ""} restant
              {tokensInfo.remainingTokens > 1 ? "s" : ""}
            </div>
          )}
        </div>
      </div>
