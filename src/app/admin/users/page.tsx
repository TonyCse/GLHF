...
export const metadata: Metadata = {
  title: "Gestion des utilisateurs | GLHF",
  description: "Administrez les utilisateurs de la plateforme GLHF.",
};
...
type SearchParams = {
  q?: string;
  p?: string;
  show?: string; // "all" pour afficher aussi les supprimes
  ok?: string;
  err?: string;
};
    ...
    <div className="space-y-6">
      ...
      {!!params?.ok && (
        <div className="rounded-lg border border-green-500/30 bg-green-500/10 text-green-200 px-3 py-2 text-sm">
          {params.ok === "soft_toggled" ? "Statut mis à jour." : params.ok}
        </div>
      )}
      {!!params?.err && params.err === "admin_role_restricted" && (
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 text-amber-200 px-3 py-2 text-sm">
          Vous êtes admin. La gestion des rôles est réservée au super admin.
        </div>
      )}
      {!!params?.err && params.err !== "admin_role_restricted" && (
          ...
          <tbody>
            {rows.map((u) => {
              ...
              return (
                <tr
                  key={u.id}
                  className={`border-t border-[#2a2c30] ${u.isDeleted ? "opacity-60" : ""}`}
                >
                  <td className="p-4 text-white">{u.id}</td>
                  <td className="p-4">
                    <span className={u.isDeleted ? "line-through" : ""}>{u.pseudo}</span>
                  </td>
                  ...
                  <td className="p-4">
                    <span
                      className={`inline-flex items-center rounded-full px-3 py-1 text-sm font-medium ${
                        u.isDeleted ? "bg-[#2a2c30] text-white" : "bg-[#232426] text-white"
                      }`}
                    >
                    ...
                    >
                      <button
                        className={`rounded-lg px-4 py-2 text-sm border ${
                          !canManageDelete
                            ? "border-[#2a2c30] text-white cursor-not-allowed opacity-60"
                            : u.isDeleted
      ...
      <div className="lg:hidden space-y-4">
        {rows.map((u) => {
          ...
          return (
            ...
            >
              {/* Entete avec pseudo et badges */}
              <div className="bg-[#232426] p-4 border-b border-[#2a2c30]">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                  <div className="flex items-center gap-2 min-w-0">
                    <span
                      className={`text-lg font-semibold text-white truncate ${u.isDeleted ? "line-through" : ""}`}
                    >
                      {u.pseudo}
                    </span>
                    <span className="text-xs text-white shrink-0">#{u.id}</span>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    ...
                    <span
                      className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-medium ${
                        u.isDeleted ? "bg-red-600 text-red-100" : "bg-green-600 text-green-100"
                      }`}
                    >
                      {u.isDeleted ? "Supprimé" : "Actif"}
                    </span>
              ...
              <div className="p-4 space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 text-sm">
                    <span className="text-white font-medium min-w-0">Email:</span>
                    <span className="text-white break-all sm:truncate">{u.email}</span>
                  </div>
                  <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 text-sm">
                    <span className="text-white font-medium">Inscrit le:</span>
        ...
        {rows.length === 0 && (
          <div className="rounded-xl bg-[#1c1d1f] border border-[#2a2c30] p-8 text-center text-white">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 text-base">
            ...
            {page > 1 && (
              <Link
                ...
                aria-label="Page précédente de la liste des utilisateurs"
                className="rounded-lg px-4 py-2 border border-[#2a2c30] text-white hover:border-[#8F60D0] hover:text-[#8F60D0] transition-colors"
              >
                ← Précédent
              </Link>
            )}
            ...
            {Array.from({ length: Math.min(pages, 7) }, (_, i) => {
              ...
              return (
                <Link
                  ...
                  className={`rounded-lg px-4 py-2 border transition-colors min-w-11 text-center ${
                    isActive
            ...
            {page < pages && (
              <Link
                ...
                aria-label="Page suivante de la liste des utilisateurs"
                className="rounded-lg px-4 py-2 border border-[#2a2c30] text-white hover:border-[#8F60D0] hover:text-[#8F60D0] transition-colors"
              >
                Suivant →
              </Link>
