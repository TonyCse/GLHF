...
export const metadata: Metadata = {
  title: "Administration | GLHF",
  description: "Tableau de bord d'administration GLHF.",
  alternates: {
    canonical: "/admin",
  },
  robots: {
...
function AdminStatCard({
  ...
  return (
    <div className="rounded-2xl p-5 bg-[#1c1d1f] shadow border border-[#2a2c30] hover:border-[#8F60D0]/30 transition-all hover:scale-105">
      ...
      {trend && (
        <div
          className={`flex items-center gap-1 mt-2 text-base ${trend.isPositive ? "text-green-400" : "text-red-400"}`}
        >
...
// Build le tableau de bord admin.
export default async function AdminDashboard() {
  ...
  */
  ...
  const tournamentCompletionRate =
    tournaments > 0 ? ((finishedTournaments / tournaments) * 100).toFixed(1) : "0";
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4 text-2xl">
          ...
          <div className="text-white">Dernière maj: {new Date().toLocaleTimeString("fr-FR")}</div>
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6">
        <AdminStatCard
          ...
          subtitle={`${deletedUsers} supprimés`}
          color="text-blue-400"
          trend={{ value: recentUsers, isPositive: recentUsers > 0 }}
        />
        <AdminStatCard
          title="Admins"
          value={admins}
          subtitle={`${((admins / users) * 100).toFixed(1)}% du total`}
          color="text-purple-400"
        />
        <AdminStatCard
          title="Tournois Actifs"
          value={activeTournaments}
          subtitle={`${deletedTournaments} supprimés`}
          color="text-yellow-400"
          trend={{ value: recentTournaments, isPositive: recentTournaments > 0 }}
        />
        <AdminStatCard
          title="Tournois Terminés"
          value={finishedTournaments}
          subtitle={`${tournamentCompletionRate}% terminés`}
          color="text-green-400"
        />
        <AdminStatCard
          title="Participations"
          value={totalParticipations}
          subtitle={`${avgParticipationsPerUser} par utilisateur`}
          color="text-cyan-400"
        />
        <AdminStatCard
          title="Matches"
          value={totalMatches}
          subtitle={`${finishedTournaments > 0 ? (totalMatches / finishedTournaments).toFixed(1) : "0"} par tournoi`}
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <AdminStatCard
          ...
          value={`${users > 0 ? ((totalParticipations / users) * 100).toFixed(1) : "0"}%`}
          subtitle="Participations vs Utilisateurs"
          color="text-emerald-400"
        />
        <AdminStatCard
          title="Tournois/Utilisateur"
          value={avgTournamentsPerUser}
          subtitle="Moyenne de création"
          color="text-rose-400"
        />
        <AdminStatCard
      ...
      <div className="grid gap-6 lg:grid-cols-2 xl:grid-cols-3">
        ...
        <InteractivePieChart
          ...
          title="Top créateurs de tournois"
          enableUserClick={true}
        />
        <InteractivePieChart
          data={winnersChartData}
          title="Top gagnants de tournois"
          enableUserClick={true}
        />
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="rounded-2xl p-6 bg-[#1c1d1f] border border-[#2a2c30]">
          <h3 className="text-2xl font-medium text-white mb-4 flex items-center gap-2">
            <span aria-hidden="true">🚨</span> Alertes &amp; Notifications
          </h3>
            ...
            {deletedTournaments > 0 && (
              <div className="p-3 rounded-lg bg-amber-600/10 border border-amber-600/30 text-amber-300">
                ...
                <div className="text-base opacity-80">
                  {deletedTournaments} tournois en soft delete
                </div>
              </div>
            )}
            {recentUsers > 5 && (
              <div className="p-3 rounded-lg bg-green-600/10 border border-green-600/30 text-green-300">
                ...
                <div className="text-base opacity-80">
                  {recentUsers} nouveaux utilisateurs cette semaine
                </div>
              </div>
            )}
        ...
        <div className="rounded-2xl p-6 bg-[#1c1d1f] border border-[#2a2c30]">
          <h3 className="text-2xl font-medium text-white mb-4 flex items-center gap-2">
            <span aria-hidden="true">📊</span> Résumé des performances
          </h3>
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="text-white font-medium text-2xl">
                {gameChartData.length > 0
                  ? gameChartData.reduce((a, b) => (a.value > b.value ? a : b)).label
                  : "N/A"}
              </span>
            </div>
          </div>
        ...
        <div className="rounded-2xl p-6 bg-[#1c1d1f] border border-[#2a2c30]">
          ...
          <div className="space-y-3">
            <Link
              href="/admin/users"
              className="block w-full text-center rounded-lg bg-[#8F60D0] hover:bg-[#A855F7] px-4 py-3 text-2xl font-medium transition-colors"
            >
              Gérer les utilisateurs
            </Link>
            <Link
              href="/admin/tournois"
              className="block w-full text-center rounded-lg border border-[#2a2c30] hover:border-[#8F60D0] hover:bg-[#8F60D0]/10 px-4 py-3 text-2xl transition-colors"
            >
              Voir les tournois
            </Link>
            {isSuperAdmin && (
