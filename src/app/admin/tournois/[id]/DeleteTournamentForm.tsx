...
export default function DeleteTournamentForm({ tournamentId }: Props) {
  ...
  return (
    <form method="POST" action={`/api/admin/tournaments/${tournamentId}/toggle-delete`} onSubmit={handleSubmit}>
      <button
        className="w-full block text-center rounded-lg border border-red-600/40 text-red-300 hover:border-red-500 hover:text-red-200 px-4 py-2.5 text-sm font-medium transition-colors cursor-pointer"
        type="submit"
      >
